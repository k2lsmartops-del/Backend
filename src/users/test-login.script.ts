...
/**
 * Script de diagnostic pour tester la connexion des utilisateurs importés.
 * 
 * Usage: npx ts-node src/users/test-login.script.ts <phone> <password>
 * Exemple: npx ts-node src/users/test-login.script.ts 0700000002 Passw0rd2
 */
...
async function testLogin(phone: string, password: string) {
  ...
  try {
    ...
    if (!isValid) {
      // Test avec variations courantes
      ...
      // Test re-hash du mot de passe fourni
